...
// Keep level13's base speed
const INITIAL_GAME_SPEED = 120;

// Update GAME_ELEMENTS to match level12's powers
const GAME_ELEMENTS = {
  SNAKE_HEAD: "🟢",
  SNAKE_BODY: "🟩",
  FOOD: "🍎",
...
const Level13 = () => {
  ...
  // Keep level13's restricted areas
  const [restrictedAreas, setRestrictedAreas] = useState([
    { x: 8, y: 8, width: 4, height: 4 }, // Center restricted area
  ]);
  ...
  // Modify moveSnake to improve power-up collision detection
  const moveSnake = async () => {
    ...
    // Check for power-up collision with improved collision detection
    if (powerUp && 
        Math.abs(head.x - powerUp.position.x) < 1 && 
        Math.abs(head.y - powerUp.position.y) < 1) {
  ...
  const restartGame = () => {
    ...
    // Reset obstacles to stationary positions
    setObstacles([
      { x: 5, y: 5 },
      { x: 15, y: 5 },
      { x: 7, y: 7 },
  ...
  const handlePowerUp = async (type: string) => {
    ...
    // Immediately apply power effects
    switch (type) {
      case "SHIELD":
        setIsShielded(true);
        break;
      case "SPEED_UP":
        setGameSpeed(prev => Math.max(prev * 0.7, INITIAL_GAME_SPEED * 0.7));
        break;
      case "SPEED_DOWN":
        setGameSpeed(prev => Math.min(prev * 1.3, INITIAL_GAME_SPEED * 1.3));
        break;
      case "SCORE_BOOST":
  ...
  // Modify updatePowerDuration for better power management
