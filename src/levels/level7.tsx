...
// Keep level7's base speed
const INITIAL_GAME_SPEED = 180;
...
const Level7 = () => {
  ...
  // Keep level7's obstacles with varying speeds
  const [obstacles, setObstacles] = useState([
    // Three vertical moving obstacles with varying speeds
    { x: 5, y: 5, direction: { x: 0, y: 1 }, speed: 1.2 },
    { x: 15, y: 5, direction: { x: 0, y: 1 }, speed: 1.4 },
    { x: 10, y: 10, direction: { x: 0, y: -1 }, speed: 1.3 },

    // Two horizontal moving obstacles
    { x: 8, y: 15, direction: { x: 1, y: 0 }, speed: 1.5 },
    { x: 12, y: 8, direction: { x: -1, y: 0 }, speed: 1.3 },

    // Fewer but strategically placed stationary obstacles
    { x: 3, y: 3, direction: { x: 0, y: 0 } },
    { x: 17, y: 17, direction: { x: 0, y: 0 } },
    { x: 3, y: 17, direction: { x: 0, y: 0 } },
    { x: 17, y: 3, direction: { x: 0, y: 0 } },
  ]);
  ...
  // Keep level7's obstacle movement logic with varying speeds
  useEffect(() => {
    if (!isPlaying || isPaused) return;
    ...
    const moveObstacles = setInterval(() => {
      setObstacles((currentObstacles) =>
        currentObstacles.map((obstacle) => {
          ...
          // Bounce off walls with adjusted positions
    ...
    }, gameSpeed * 1.2); // Changed from 1.5 to 1.2 for even faster obstacle movement
  ...
  const restartGame = () => {
    ...
    // Reset obstacles to level7's specific pattern
    setObstacles([
      { x: 5, y: 5, direction: { x: 0, y: 1 }, speed: 1.2 },
      { x: 15, y: 5, direction: { x: 0, y: 1 }, speed: 1.4 },
      { x: 10, y: 10, direction: { x: 0, y: -1 }, speed: 1.3 },
  ...
  // Keep level7's getValidFoodPosition helper
