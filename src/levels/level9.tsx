...
// Keep level9's base speed
const INITIAL_GAME_SPEED = 160;
...
const Level9 = () => {
  ...
  // Keep level9's obstacles with varying speeds
  const [obstacles, setObstacles] = useState([
    ...
    // Diagonal moving obstacle (slower)
  ...
  // Keep teleporter states and logic
  ...
  const moveSnake = async () => {
    ...
    if (!teleportCooldown) {
      ...
      if (teleporterHit) {
        const otherTeleporter = teleporters.find(t => t.id !== teleporterHit.id);
        if (otherTeleporter) {
          head = {
            x: otherTeleporter.x + direction.x,
            y: otherTeleporter.y + direction.y,
          };
  ...
  const restartGame = () => {
    ...
    // Reset obstacles to level9's specific pattern
    setObstacles([
      // Four vertical moving obstacles with slower speeds
      { x: 5, y: 5, direction: { x: 0, y: 1 }, speed: 0.8 },
      { x: 15, y: 5, direction: { x: 0, y: 1 }, speed: 0.8 },
