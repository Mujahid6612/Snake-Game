...
// Keep level4's base speed
const INITIAL_GAME_SPEED = 210;
...
const Level4 = () => {
  ...
  // Keep level4's obstacles
  const [obstacles, setObstacles] = useState([
    { x: 10, y: 8, direction: { x: 1, y: 0 } },
    { x: 5, y: 12, direction: { x: 0, y: 1 } },
    { x: 15, y: 15, direction: { x: -1, y: 0 } },
  ]);
  ...
  // Add obstacle movement effect from level4
  useEffect(() => {
    if (!isPlaying || isPaused) return;
    ...
    }, gameSpeed * 1.5); // Slightly slower than snake speed
