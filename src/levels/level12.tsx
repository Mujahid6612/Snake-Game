...
// Keep level12's base speed
const INITIAL_GAME_SPEED = 130;

// Keep level12's GAME_ELEMENTS with all powers
const GAME_ELEMENTS = {
  SNAKE_HEAD: "🟢",
  SNAKE_BODY: "🟩",
  FOOD: "🍎",
  ...
  TIME_FREEZE: { emoji: "❄️", name: "Time Freeze", size: 18, duration: 15 },
  GHOST: { emoji: "👻", name: "Ghost Mode", size: 18, duration: 15 },
};
...
const Level12 = () => {
  ...
  // Keep level12's obstacles with varying speeds
  ...
  useEffect(() => {
    if (isPlaying) {
      const spawnPowerUp = () => {
        if (Math.random() < 0.8) {
          const types = [
            "SPEED_UP", "SPEED_DOWN", "SCORE_BOOST", "WALL_PASS",
            "SHIELD", "TIME_FREEZE", "GHOST"
          ];
  ...
  useEffect(() => {
    // Time Freeze halts every moving obstacle while it's active.
    if (!isPlaying || isPaused || obstaclesFrozen) return;
  ...
  }, [isPlaying, isPaused, obstaclesFrozen]);
  ...
  const moveSnake = async () => {
    ...
    // Improve collision detection by using a smaller threshold
    const checkCollision = (pos1: any, pos2: any, threshold = 0.8) => {
      return Math.abs(pos1.x - pos2.x) < threshold && Math.abs(pos1.y - pos2.y) < threshold;
    };

    // Check for obstacle collision with shield and ghost mode protection
    const hitObstacle = obstacles.some((obstacle) => 
      checkCollision(obstacle, head) && !isShielded && !isGhostMode
    );
    ...
    // Improve power-up collision detection
    if (powerUp && checkCollision(head, powerUp.position)) {
      const powerType = powerUp.type;
      setPowerUp(null);
      await handlePowerUp(powerType);
    }

    // Check for food collision with improved detection
  ...
  const restartGame = () => {
    ...
    // Reset obstacles to level12's specific pattern
    setObstacles([
      // Four vertical moving obstacles with slower speeds
      { x: 5, y: 5, direction: { x: 0, y: 1 }, speed: 0.8 },
      { x: 15, y: 5, direction: { x: 0, y: 1 }, speed: 0.8 },
  ...
  // Keep every power effect in sync with the active power-ups so each one
  // ends exactly when its timer runs out (fixes powers lingering after expiry,
  // even when several overlap).
  useEffect(() => {
    setIsShielded(activePowers.some((p) => p.type === "SHIELD"));
    setObstaclesFrozen(activePowers.some((p) => p.type === "TIME_FREEZE"));
    setIsGhostMode(activePowers.some((p) => p.type === "GHOST"));
  ...
  // Improve handlePowerUp to ensure powers are properly activated
  const handlePowerUp = async (type: string) => {
    ...
    // Find existing power and calculate new duration
    ...
    // Immediately apply power effect
    switch (type) {
      case "SHIELD":
        setIsShielded(true);
        break;
      case "TIME_FREEZE":
        setObstaclesFrozen(true);
        break;
      case "GHOST":
        setIsGhostMode(true);
        break;
      case "SPEED_UP":
        setGameSpeed(prev => prev * 0.7);
        break;
      case "SPEED_DOWN":
        setGameSpeed(prev => prev * 1.3);
        break;
      case "SCORE_BOOST":
    ...
    // Update power duration and active powers list
    updatePowerDuration(type, totalDuration);
  };

  // Improve updatePowerDuration to handle power expiration properly
  const updatePowerDuration = (type: string, duration: number) => {
    setActivePowers(prev => {
      ...
      if (existingPowerIndex !== -1) {
        ...
        updatedPowers[existingPowerIndex] = newPower;
        return updatedPowers;
      }
      return [...prev, newPower];
    });

    // Set timeout to remove power effect
  ...
  // Modify getValidFoodPosition to remove teleporter check
